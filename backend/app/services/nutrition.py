...
# ── Table nutritionnelle (kcal, prot, carbs, fat, fiber — valeurs pour portion moyenne ~100 g) ─
NUTRITION_TABLE: dict[str, dict[str, float]] = {
    "poulet grillé": {"kcal": 165, "prot": 31, "carbs": 0, "fat": 3.6, "fiber": 0},
    "poulet": {"kcal": 165, "prot": 31, "carbs": 0, "fat": 3.6, "fiber": 0},
    ...
    "steak": {"kcal": 250, "prot": 26, "carbs": 0, "fat": 15, "fiber": 0},
    "poisson": {"kcal": 120, "prot": 22, "carbs": 0, "fat": 3, "fiber": 0},
    "légumes": {"kcal": 35, "prot": 2, "carbs": 7, "fat": 0.3, "fiber": 3},
}

DEFAULT_NUTRITION = {"kcal": 150, "prot": 8, "carbs": 20, "fat": 5, "fiber": 2}

PORTION_MULTIPLIERS = {"petite": 0.7, "moyenne": 1.0, "grande": 1.3}

# Aligné sur client/src/services/healthProfile.ts
ALLERGEN_KEYWORDS: dict[str, list[str]] = {
    "gluten": ["gluten", "blé", "seigle", "orge", "épeautre", "pâtes", "pasta", "pain", "pizza"],
    "lactose": ["lait", "lactose", "lactosérum", "caséine", "crème", "fromage", "beurre"],
    "eggs": ["œuf", "oeuf", "albumine", "omelette"],
    "nuts": ["amande", "noisette", "noix", "pistache", "cajou"],
    "peanuts": ["arachide", "cacahuète", "cacahuete"],
    "soy": ["soja", "tofu", "edamame"],
    "fish": ["poisson", "anchois", "thon", "saumon", "sardine"],
    "shellfish": ["crevette", "crabe", "homard", "crustacé", "moule"],
    "sulfites": ["sulfite"],
}
...
def _build_warnings(foods: list[dict], profile: Optional[dict]) -> list[str]:
    ...
    warnings: list[str] = []
    names = _food_names_lower(foods)
    combined = " ".join(names)

    for allergen_id in profile.get("allergens", []):
        keywords = ALLERGEN_KEYWORDS.get(allergen_id, [_normalize(allergen_id)])
        match = _contains_keyword(names, keywords)
        if match:
            warnings.append(
                f"Allergie ({allergen_id}) : présence possible de « {match} » dans le repas — "
                "vérifiez les ingrédients avant consommation."
            )

    if profile.get("diabetes"):
        carb_match = _contains_keyword(names, HIGH_CARB_KEYWORDS)
        if carb_match:
            warnings.append(
                "Diabète : repas riche en glucides — surveillance glycémique et "
                "adaptation des portions recommandées."
            )

    if profile.get("hypertension"):
        salt_match = _contains_keyword(names, HIGH_SODIUM_KEYWORDS)
        if salt_match:
            warnings.append(
                "Hypertension : aliment potentiellement salé détecté — limitez le sodium "
                "et privilégiez les assaisonnements sans sel."
            )
    ...
    if profile.get("obesity"):
        ...
        if macros["kcal"] >= 700:
            warnings.append(
                f"Objectif poids : repas estimé à {macros['kcal']:.0f} kcal — "
                "envisagez une portion réduite ou plus de légumes."
            )

    diet = profile.get("diet", "none")
    if diet in ("vegetarian", "vegan"):
        animal_match = _contains_keyword(names, ANIMAL_KEYWORDS)
        vegan_keywords = ANIMAL_KEYWORDS + (VEGAN_EXTRA_KEYWORDS if diet == "vegan" else [])
        animal_match = _contains_keyword(names, vegan_keywords)
        if animal_match:
            label = "vegan" if diet == "vegan" else "végétarien"
            warnings.append(
                f"Régime {label} : « {animal_match} » détecté — ce repas peut ne pas "
                "correspondre à votre régime."
            )

    if diet == "gluten-free":
        gluten_match = _contains_keyword(names, ALLERGEN_KEYWORDS["gluten"])
        if gluten_match:
            warnings.append(
                "Sans gluten : aliment à risque détecté — vérifiez l'absence de gluten."
            )

    if diet == "halal":
        pork_match = _contains_keyword(names, PORK_KEYWORDS)
        if pork_match:
            warnings.append(
                f"Régime halal : « {pork_match} » détecté — produit non conforme au régime halal."
            )
    ...
    if diet == "kosher":
        ...
        if pork_match:
            warnings.append(
                f"Régime kosher : « {pork_match} » détecté — produit non casher."
            )

        shellfish_match = _contains_keyword(names, KOSHER_SHELLFISH_KEYWORDS)
        if shellfish_match:
            warnings.append(
                f"Régime kosher : « {shellfish_match} » détecté — fruits de mer non casher "
                "(crustacés et mollusques interdits)."
            )

        has_meat = _contains_keyword(names, KOSHER_MEAT_KEYWORDS)
        has_dairy = _contains_keyword(names, KOSHER_DAIRY_KEYWORDS)
        if has_meat and has_dairy:
            warnings.append(
                "Régime kosher : mélange viande et produit laitier détecté dans le même repas — "
                "non conforme aux règles casher (séparation viande/lait)."
            )
